...
(function() {
  var $, applyImageWatermark, applyTextWatermark, applyWatermarkToPDFPage, autoRefresh, canvas, checkPDFJSLibrary, clearSelectionBtn, createErrorPlaceholder, createFileBlob, createPDFBlob, createPagePreview, currentPDFPages, dataURItoBlob, downloadAsImages, downloadAsPDF, downloadSelectedPages, downloadSelectedPagesBtn, drawImageWatermark, drawImageWatermarkOnCanvas, drawText, drawTextWatermarkOnCanvas, file, fileInput, fileUploadLabel, generateFileName, getFontFamily, graph, hideMobileLoadingTip, image, imageWatermarkOptions, imageWatermarkRadio, initializePreviewArea, input, inputItems, insertPageInOrder, isMobile, isProcessing, isUpdatingPreview, lastUpdateTime, makeStyle, originalFileName, originalFileType, pdfDocument, pdfPageCountSpan, pdfPagesContainer, pdfPagesDiv, previewSelectedPages, previewSelectedPagesBtn, processedCanvases, rangeInputs, readFile, readImageFile, readPDFFile, redraw, refresh, refreshMultiPagePreview, refreshSinglePreviewPage, renderPagesSequentially, renderPreviewPages, rerenderPDFPages, rerenderSinglePDFPage, selectAllPagesBtn, selectedPageCountSpan, selectedPages, setupCanvas, showMobileLoadingTip, state, textCtx, textInput, textWatermarkOptions, textWatermarkRadio, updatePreviewArea, updateRangeValues, updateSelectedPageCount, updateTimeout, watermarkImage, watermarkImageFile, watermarkType,
    indexOf = [].indexOf;
  ...
  readImageFile = function() {
    ...
    fileReader.onload = function() {
      ...
      img.onerror = function() {
        ...
        return alert('图片文件格式不支持或已损坏');
      };
      return img.src = fileReader.result;
    };
    fileReader.onerror = function() {
      hideMobileLoadingTip();
      return alert('图片文件读取失败，请检查文件是否损坏');
    };
  ...
  drawImageWatermarkOnCanvas = function(targetCanvas, ctx) {
    ...
    watermarkReader.onload = function() {
      ...
      watermarkImg.onerror = function() {
        return console.error('水印图片加载失败');
      };
      return watermarkImg.src = watermarkReader.result;
    };
    watermarkReader.onerror = function() {
      return console.error('水印图片文件读取失败');
    };
...
<ul>
  ...
  if (isMobile()) {
    ...
    rangeInputs.forEach(function(input) {
      return input.addEventListener('touchstart', function(e) {
        // 防止页面滚动
    ...
    if (pdfPagesContainer != null) {
      pdfPagesContainer.addEventListener('touchstart', function(e) {
        // 防止触摸时的意外滚动
  ...
  window.addEventListener('orientationchange', function() {
    return setTimeout(function() {
      ...
      // 重新渲染预览（如果存在）
      if (state.currentPDF && selectedPages.size > 0) {
        return updatePreviewArea();
      }
    }, 100);
  });
  ...
  // 添加加载状态提示（移动端网络可能较慢）
  showMobileLoadingTip = function(message) {
    var tip;
    if (isMobile()) {
      tip = document.createElement('div');
      tip.className = 'mobile-loading-tip';
      tip.innerHTML = `<div style="
    position: fixed;
    top: 50%;
    left: 50%;
